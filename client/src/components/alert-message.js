...
export default function AlertMessage() {
  ...
  const alertMessage = (
    <StyledAlertMessage className={"alert floating alert-server alert-" + messageObject.severity} onClose={onCloseAlertMessage} role="alert">
      <StyledAlertMessageIcon fontName={messageObject.icon} ></StyledAlertMessageIcon>
      <StyledAlertMessageText>{ messageObject.message }</StyledAlertMessageText>
      <StyledAlertMessageCloseButton data-bs-dismiss="alert" aria-label="Close" onClick={onCloseAlertMessage} close ></StyledAlertMessageCloseButton>
    </StyledAlertMessage>
  );
